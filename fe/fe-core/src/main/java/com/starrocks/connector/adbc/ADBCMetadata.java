 ...
 *
 * <h3>Hierarchy state machine</h3>
 * <pre>
 *   null  --[first metadata call: resolveHierarchy()]--&gt;  CATALOG | SCHEMA | UNKNOWN
 *
 *   UNKNOWN  --[first getTable: getTableSchemaProbing() succeeds]--&gt;  SCHEMA | CATALOG
 *           (one-time upgrade; the driver implements getTableSchema even though
 *            getObjects threw NOT_IMPLEMENTED, so we learn the model lazily)
 *
 *   any state  --[clear()]--&gt;  null   (forces re-resolution on next call)
 * </pre>
 *
 * <p>Terminal states ({@link HierarchyModel#CATALOG} and {@link HierarchyModel#SCHEMA})
 * never transition again until {@code clear()}. {@link HierarchyModel#UNKNOWN} is the
 * only non-terminal state; it exists so the per-call hot path stays simple while the
 * probe-and-upgrade work happens lazily inside {@code getTable}.
 */
public class ADBCMetadata implements ConnectorMetadata {
    ...
    private final ConcurrentHashMap<ADBCTableName, Long> tableIdMap;
    ...
    private List<String> listDbNamesFromGetObjects(AdbcConnection conn, HierarchyModel model)
        ...
        if (model == HierarchyModel.UNKNOWN) {
            // resolveHierarchy() already saw NOT_IMPLEMENTED on this driver;
            // skip the wasted RPC and go straight to SQL discovery.
    ...
    public Table getTable(ConnectContext context, String dbName, String tblName) {
        ...
        try {
            ...
            long tableId = tableIdMap.computeIfAbsent(key,
                    k -> ConnectorTableId.CONNECTOR_ID_GENERATOR.getNextId().asLong());
    ...
    /**
     * Probe both schema and catalog positions when the hierarchy model is UNKNOWN.
     * Tries schema first (more common — PostgreSQL/DuckDB), then catalog (SQLite),
     * then falls back to SQL inference.
     *
     * <p>On the first successful probe, {@code hierarchyModel} is upgraded so
     * subsequent calls bypass this method entirely.
     */
    private Schema getTableSchemaProbing(AdbcConnection conn, String dbName, String tblName)
            throws Exception {
        Schema schema = tryProbeAndUpgrade(conn, /*catalogArg*/ null, /*schemaArg*/ dbName,
                tblName, HierarchyModel.SCHEMA);
        if (schema != null) {
            return schema;
        }

        schema = tryProbeAndUpgrade(conn, /*catalogArg*/ dbName, /*schemaArg*/ null,
                tblName, HierarchyModel.CATALOG);
        if (schema != null) {
            return schema;
        }

        LOG.warn("ADBC catalog '{}': getTableSchema failed at both schema and catalog positions, "
                + "falling back to query-based inference for {}.{}", catalogName, dbName, tblName);
        return getTableSchemaViaQuery(conn, dbName, tblName);
    }

    /**
     * Single getTableSchema attempt at one hierarchy position. On success,
     * promotes {@code hierarchyModel} to {@code upgradeTo} so future calls
     * skip the probe.
     *
     * <p>{@code NOT_IMPLEMENTED} and {@code NOT_FOUND} are treated as "wrong position,
     * try the next one"; every other AdbcException is rethrown so real errors
     * (network failure, auth failure) surface to the caller.
     */
    private Schema tryProbeAndUpgrade(AdbcConnection conn,
                                       String catalogArg, String schemaArg, String tblName,
                                       HierarchyModel upgradeTo) throws Exception {
        boolean asSchema = upgradeTo == HierarchyModel.SCHEMA;
        String position = asSchema ? "schema" : "catalog";
        String value = asSchema ? schemaArg : catalogArg;
        try {
            Schema schema = conn.getTableSchema(catalogArg, schemaArg, tblName);
            if (schema != null) {
                LOG.info("ADBC catalog '{}': getTableSchema succeeded with {}='{}', "
                        + "upgrading hierarchy to {}", catalogName, position, value, upgradeTo);
                // hierarchyModel is volatile; a single write is atomic. If two probes
                // race, both write the same value (driver behavior is deterministic).
                hierarchyModel = upgradeTo;
            }
            return schema;
        } catch (AdbcException e) {
            if (e.getStatus() != AdbcStatusCode.NOT_IMPLEMENTED
            ...
            LOG.debug("getTableSchema({}='{}') missed: {}", position, value, e.getMessage());
            return null;
        }
    }
    ...
    public void clear() {
        tableIdMap.clear();
        // hierarchyModel is volatile; a single write is enough. Resetting to null
        // forces the next metadata call to re-run resolveHierarchy().
        hierarchyModel = null;
    }
